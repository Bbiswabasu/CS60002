...
def add():
    ...
    for i in range(numberOfServers):
        ...
        res=os.popen(f'sudo docker run --name {hostName} --network pub --network-alias {hostName} -e serverId={g.newServerId} -d ds_server:latest').read()
        ...
        if len(res)==0:
            ...
            return {"message": "Unable to create instance!"}, 500
        else:
